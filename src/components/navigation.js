...
const Navigation = ({ menuLinks }) => {
  ...
  const dropdownLinks = menuLinks.filter(link => dropdownLinkNames.includes(link.name.toUpperCase()));
      ...
      <nav>
        <ul style={{display: "flex", flexWrap: 'wrap', paddingLeft: '1rem', paddingRight: '1rem', paddingTop: '1.68rem', paddingBottom: 0}}>
          <li style={{ listStyleType: `none`, paddingLeft: '1rem', paddingRight: '1rem', position: 'relative' }} onMouseEnter={toggleDropdown} onMouseLeave={toggleDropdown}>
            <div style={{color: `#FFFFFF`, textDecoration: `none`, fontSize: `.73rem`, fontWeight: `700`}}>
            <LinkRouter to={'/'} style={{color: `#FFFFFF`, textDecoration: `none`, fontSize: `.73rem`, fontWeight: `700`}}>
              HOME
            </LinkRouter>
            </div>
          ...
          {otherLinks.map(link => (
            <li key={link.name} style={{ listStyleType: `none`, paddingLeft: '1rem', paddingRight: '1rem'}}>
              <LinkRouter to={link.link} style={{color: `#FFFFFF`, textDecoration: `none`, fontSize: `.73rem`, fontWeight: `700`}}>
                {link.name}
              </LinkRouter>
            </li>
          ))}
